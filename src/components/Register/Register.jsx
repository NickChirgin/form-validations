...
const Register = ({closeHandler}) => {
  ...
  return (
    <form className={style.register}>
      ...
      {data.map(input => 
      <FormInput key={input.id} {...input} onChange={changeHandler} pattern={input.name === "confirmPassword" ? value.password : input.pattern}/>
        )}
      <button type='submit' className={style.register__button}>Register</button>
      <p className={style.register__login}>Already have an account? <span className={style.register__span}>Login</span></p>
    </form>
